  ...
  return useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select(`
          *,
          cliente:clientes(id, nombre, codigo_cliente, dni_cuit, direccion, telefono, localidad, zona_id, vendedor_id, zona:zonas(id, nombre)),
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      await supabase.from('pedido_historial').insert({
        ...
        usuario_id: user.id,
        observaciones: 'Pedido creado en pendiente'
      });
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Pedido creado en pendiente' });
    },
    onError: (error) => {
      toast({ 
