...
export default function POS() {
  ...
  const [actualizandoPicking, setActualizandoPicking] = useState(false);

  const handleActualizarYReimprimir = async () => {
    if (!editingPedidoId) return;
    if (cart.length === 0) {
      toast.error('El pedido no puede quedar vacío');
      return;
    ...
    try {
      ...
      bumpPedidosPanel();
      toast.success('Picking actualizado y reimpreso');
    } catch (e: any) {
      toast.error(e?.message || 'No se pudo actualizar el picking');
    } finally {
      setActualizandoPicking(false);
    }
  };
