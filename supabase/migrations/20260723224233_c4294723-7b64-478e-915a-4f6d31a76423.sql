CREATE OR REPLACE FUNCTION public.resolver_nota_credito(p_comprobante_nc_id uuid, p_caja_id uuid, p_cliente_id uuid, p_monto_caja numeric, p_monto_cc numeric, p_factura_compra_mov_id uuid, p_concepto_caja text, p_concepto_cc text, p_venta_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF COALESCE(p_monto_caja, 0) > 0 THEN
    ...
    -- Validar que la caja esté abierta Y pertenezca al usuario que invoca
    PERFORM 1 FROM public.cajas
     ...
     FOR UPDATE;
    IF NOT FOUND THEN
      RAISE EXCEPTION 'La caja indicada no está abierta o no pertenece al usuario que emite la NC';
    END IF;
