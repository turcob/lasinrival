...
CREATE OR REPLACE FUNCTION public.get_ventas_lista(
  p_estado text DEFAULT 'confirmada'::text,
  p_usuario_id uuid DEFAULT NULL::uuid,
  p_vendedor_id uuid DEFAULT NULL::uuid,
  p_sin_vendedor boolean DEFAULT false,
...
RETURNS TABLE(
  id uuid, es_pedido boolean, numero_comprobante integer, fecha timestamp with time zone,
  subtotal numeric, descuento numeric, total numeric, anulada boolean, motivo_anulacion text,
  fecha_anulacion timestamp with time zone, usuario_id uuid, cliente_id uuid, caja_id uuid,
  estado text, pedido_id uuid, tipo_pedido text, pedido_estado text, origen text,
  cliente_nombre text, cliente_dni_cuit text, cliente_condicion_iva integer,
  cliente_vendedor_id uuid, usuario_nombre text, afip jsonb, pagos jsonb, total_count bigint
)
LANGUAGE sql
...
base AS (
  ...
  WHERE (SELECT is_priv FROM ctx)
     ...
     OR v.vendedor_id = (SELECT uid FROM ctx)
