CREATE OR REPLACE FUNCTION public.pos_actualizar_pedido_estado(p_venta_id uuid, p_nuevo_estado text, p_detalles jsonb DEFAULT NULL::jsonb, p_venta jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF (v_venta.estado = 'pedido' AND p_nuevo_estado = 'en_preparacion')
     ...
     OR (v_venta.estado = 'en_preparacion' AND p_nuevo_estado = 'en_preparacion')
     OR (v_venta.estado = 'preparado' AND p_nuevo_estado = 'en_preparacion')
     OR (v_venta.estado = 'preparado' AND p_nuevo_estado = 'pedido')
     OR (v_venta.estado = 'preparado' AND p_nuevo_estado = 'preparado')
  THEN
  ...
  UPDATE public.ventas
     SET estado = p_nuevo_estado,
         ...
         preparado_at = CASE WHEN p_nuevo_estado = 'preparado' AND v_venta.estado <> 'preparado' THEN now() ELSE preparado_at END,
         preparado_por = CASE WHEN p_nuevo_estado = 'preparado' AND v_venta.estado <> 'preparado' THEN v_user ELSE preparado_por END
   WHERE id = p_venta_id;
