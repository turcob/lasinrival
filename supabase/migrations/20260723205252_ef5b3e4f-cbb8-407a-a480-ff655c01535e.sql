...
BEGIN
  ...
  -- ============ 0. LOCK SERIALIZANTE (xact-level, se libera solo en commit/rollback) ============
  -- Evita la carrera cuando dos llamadas casi simultáneas re-procesan el mismo pedido
  -- y duplican venta_pagos / movimientos_caja / movimientos_inventario.
  -- Rama de venta nueva: NO se lockea. Cada llamada crea un venta_id distinto, no hay
  -- hijos previos que borrar/re-insertar, y el histórico muestra 0 duplicados de header.
  IF p_editing_pedido_id IS NOT NULL THEN
    -- Timeout local: si el lock no se consigue en 5s, aborta con error explícito
    -- (SQLSTATE 55P03) en lugar de dejar al cajero esperando. SET LOCAL solo afecta
    -- a esta transacción y se revierte automáticamente en commit/rollback.
  ...
  IF p_editing_pedido_id IS NOT NULL THEN
    ...
    -- Idempotencia: al reprocesar un pedido, borrar pagos y movimientos previos
    -- para que un reintento (doble click, retry de red) no acumule filas duplicadas.
    DELETE FROM public.venta_pagos WHERE venta_id = v_venta_id;

    -- Revertir el impacto de movimientos de caja previos vinculados a esta venta,
    -- para que el nuevo INSERT en la sección 5 no acumule ingresos duplicados.
    ...
    -- Revertir movimientos de inventario, CC cliente/empleado, transferencias y cheques
    -- creados en un intento previo, para dejar la venta consistente antes de re-insertar.
    -- Stock: reponer lo descontado (salida) o descontar lo repuesto (entrada) previamente.
    FOR v_item IN
  ...
  -- ============ 5. MOVIMIENTO DE CAJA + actualizacion total_ventas ============
  ...
  -- ============ 6. CLIENTE_MOVIMIENTOS (CC cliente) ============
  ...
  -- ============ 7. EMPLEADO_MOVIMIENTOS (CC empleado) ============
